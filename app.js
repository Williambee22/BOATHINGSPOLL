...
function parseBandImport(text) {
  ...
  for (const raw of String(text || '').split(/\r?\n/)) {
    ...
    // Bulk format: School Name, ST
    // Split on the final comma so school names containing punctuation remain intact.
    const match = line.match(/^(.*),\s*([^,]+)$/);
    if (!match) continue;

    const name = String(match[1] || '').trim();
    const state = String(match[2] || '').trim().toUpperCase();
    if (name.length < 2 || !state) continue;

    // First occurrence wins. Duplicate school/state rows later in the paste are ignored.
    const identity = `${name.toLocaleLowerCase('en-US')}|${state}`;
    if (seen.has(identity)) continue;
    seen.add(identity);

    rows.push({ name, city: '', state });
  }
  return rows;
}
